...
대시보드는 비활성 화면까지 계산하지 않고 사이드바 선택에 따라 필요한 화면만 렌더링합니다. 메뉴는 초보자도 목적을 바로 이해할 수 있도록 `지금 시장은 어떤가요?`, `내 예산으로 어디를 살 수 있나요?`, `관심 단지 비교하기`, `갈아타려면 얼마가 필요할까요?`처럼 질문형으로 구성했습니다. 각 화면은 쉬운 결론과 핵심 지표를 먼저 보여주고, 전문 지표·차트·원본 표는 필요할 때 펼쳐보는 점진적 정보 구조를 사용합니다.

`최근 3개월 대표가격`, `가격 차이`, `분석 신뢰도`처럼 쉬운 기본 표현을 사용하며 기존 `3M`, `GAP`, 표본 상태 등 계산 정의는 용어 도움말과 상세 영역에서 확인할 수 있습니다. 화면에 표시되는 조건별 결과는 실거래 관찰 정보이며 투자 추천을 의미하지 않습니다.

갈아타기 분석에는 후보 최근 3개월 중앙값의 기준 주택 대비 `가격 프리미엄`, 3개월·6개월 GAP 방향을 함께 보는 `GAP 지속성`, 최근 12개월 중 양쪽 월 중앙값이 모두 존재하는 `월별 GAP 관측 수`를 표시합니다. 이 값들은 추천점수나 매수·매도 판단이 아닌 상대 가격과 관측 상태를 설명하는 보조정보입니다.
